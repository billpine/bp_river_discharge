...
dis.fdc = as.data.frame(cbind(ts(disE$val[disE$year<2021]),ts(disE$val[disE$year>2014&disE$year<2018]),
                              ts(disE$val[disE$year > 2017])))


#create myself (not using fdc() function)
all <- sort(dis.fdc$`ts(disE$val[disE$year < 2021])`, decreasing = T)
part <- sort(dis.fdc$`ts(disE$val[disE$year > 2014 & disE$year < 2018])`,decreasing = T)
part2 <- sort(dis.fdc$`ts(disE$val[disE$year > 2017])`, decreasing = T)

df_all <- data.frame(x = 100/length(all) * 1:length(all), y = all)
df_part_2015_2017 <- data.frame(x = 100/length(part) * 1:length(part), y = part)
df_part_2018_2020 <- data.frame(x = 100/length(part2) * 1:length(part2), y = part2)
...
lines(x = df_part_2015_2017$x, y = log(df_part_2015_2017$y), type = 'l', col = 'red', lwd = 2)
abline(h = log(5000), lwd = 2, col = 'blue', lty = 2)
legend("topright", legend = c("1930-2020", "2015-2017"), lty = 1, col = c("black", "red"))
axis(1, labels = c("0%", "20%", "40%", "60%", "80%", "100%"), at = c(0, 20, 40, 60,80, 100))

plot(x = df_all$x, y = log(df_all$y), type = 'l', lwd = 2, ylab = "Log(Flow (cf/s))", 
     xlab = "% Time flow equalled or exceeded", xaxt = 'n',main='Discharge duration')
lines(x = df_part_2018_2020$x, y = log(df_part_2018_2020$y), type = 'l', col = 'red', lwd = 2)
legend("topright", legend = c("1930-2020", "2018-2020"), lty = 1, col = c("black", "red"))
abline(h = log(5000), lwd = 2, col = 'blue', lty = 2)
axis(1, labels = c("0%", "20%", "40%", "60%", "80%", "100%"), at = c(0, 20, 40, 60, 80, 100))
...
df_part_2015_2017_2 <- df_part_2015_2017
df_part_2015_2017_2$x <- round(df_part_2015_2017_2$x, digits = 2)
df_part_2015_2017_3 <- aggregate(df_part_2015_2017_2, by = list(df_part_2015_2017_2$x), FUN = 'mean')

df_merge  <- merge(df_part_2015_2017_3, df_all3, by.x = "x", by.y = "x")

length(which(log(df_merge$y.x) < log(df_merge$y.y)))
length(df_merge$y.x)

#0.9654 (2821/2922) #this depends on which gauge and years used

#how often is the 2018-2020 line below the period of record line
df_part_2018_2020_2 <- df_part_2018_2020
df_part_2018_2020_2$x <- round(df_part_2018_2020_2$x, digits = 2)
df_part_2018_2020_3 <- aggregate(df_part_2018_2020_2, by = list(df_part_2018_2020_2$x), FUN = 'mean')

df_merge2  <- merge(df_part_2018_2020_3, df_all3, by.x = "x", by.y = "x")
