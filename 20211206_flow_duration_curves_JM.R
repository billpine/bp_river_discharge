...
disE  = dis[dis$dates>='1950-01-01' & dis$dates<='2021-12-01',]  
...
#plot using fdc() function
#fdc(dis.fdc, ylab='Flow (cf/s)', leg.txt=c('1950-2020','2005-2012'),cex.axis=1)
...
df_part_2013_2021<- data.frame(x = 100/length(part2) * 1:length(part2), y = part2)

#make plots
plot(x = df_all$x, y = log(df_all$y), type = 'l', lwd = 2, ylab = "Log(Flow (cf/s))", 
     xlab = "% Time discharge equalled or exceeded", xaxt = 'n',main='Discharge duration')
lines(x = df_part_2005_2012$x, y = log(df_part_2005_2012$y), type = 'l', col = 'red', lwd = 2)
abline(h = log(5000), lwd = 2, col = 'blue', lty = 2)
legend("topright", legend = c("1927-2021", "2005-2012"), lty = 1, col = c("black", "red"))
axis(1, labels = c("0%", "20%", "40%", "60%", "80%", "100%"), at = c(0, 20, 40, 60,80, 100))

plot(x = df_all$x, y = log(df_all$y), type = 'l', lwd = 2, ylab = "Log(Flow (cf/s))", 
     xlab = "% Time discharge equalled or exceeded", xaxt = 'n',main='Discharge duration')
lines(x = df_part_2013_2021$x, y = log(df_part_2013_2021$y), type = 'l', col = 'red', lwd = 2)
legend("topright", legend = c("1927-2021", "2013-2021"), lty = 1, col = c("black", "red"))
abline(h = log(5000), lwd = 2, col = 'blue', lty = 2)
axis(1, labels = c("0%", "20%", "40%", "60%", "80%", "100%"), at = c(0, 20, 40, 60, 80, 100))
...
df_part_2013_2021_2 <- df_part_2013_2021
df_part_2013_2021_2$x <- round(df_part_2013_2021_2$x, digits = 2)
df_part_2013_2021_3 <- aggregate(df_part_2013_2021_2, by = list(df_part_2013_2021_2$x), FUN = 'mean')

df_merge2  <- merge(df_part_2013_2021_3, df_all3, by.x = "x", by.y = "x")
